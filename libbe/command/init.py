...
""" Bugs Everywhere - Intialize in the specified directory.

Usage:
    be init

 This command initializes Bugs Everywhere support for the specified directory
 and all its subdirectories.  It will auto-detect any supported revision
 control system.  You can use "be set vcs_name" to change the vcs being used.
 ...
 version-control root.  In that case, the BE repository will be created under
 that directory, rather than the current directory or the one passed in --repo.
 Consider the following tree, versioned in Git.
   ~
   `--projectX
      |-- .git
      `-- src
...
class Init(libbe.command.Command):
    ...
    def run(self, args=None):
        args = args or []
        docopt(__doc__, argv=[Init.name] + args)
        storage = self._get_unconnected_storage()  # pylint: disable=no-member
        ...
        bug_dir = libbe.bugdir.BugDir(storage, from_storage=False)
        self.ui.storage_callbacks.set_bugdirs({bug_dir.uuid: bug_dir})
        # pylint: disable=no-member
        if bug_dir.storage.name != 'None':
            print >> self.stdout, \
